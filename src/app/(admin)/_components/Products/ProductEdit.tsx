...
export default function ProductEdit({
  ...
  const removeGallery = (item: any) => {
    console.log(item);
  };
  const handleKeyEnterTags = async (e: any) => {
    if (e.key === "Enter") {
      const { success, message, tag } = await createTagByProduct(productId, {
        tagName: inputTag,
      });
      if (!success) {
        toast.error(message);
  ...
  const handleKeyEnterCategory = async (e: any) => {
    if (e.key === "Enter") {
      const { success, message, category } = await createCategoryByProduct(
        productId,
        {
        ...
        <div className="col-md-8">
          <div className="card mb-4">
            ...
            <div className="card-body">
              ...
              <div className="mb-3">
                <label htmlFor="description" className="form-label font-small">
                  Description
                </label>
                <textarea
                  name="desc"
            ...
            <div className="card-body">
              ...
              <div className="mt-3">
                ...
                <input
                  ...
                  onKeyDown={handleKeyEnterCategory}
                  id="category"
                />
                {formData.categories && formData.categories.length > 0 && (
                  <div className="listTags listCategories">
                    {formData.categories.map((category: any) => {
                      return (
                        <div className="item-tag" key={category.id}>
